...
def preprocess_dataset(
    ...
    def filter_by_token_limit(dataset, max_tokens):
        acc_tokens = 0
        cur_idx = 1
        for i, sample in enumerate(dataset, start=1):
            acc_tokens += len(sample['input_ids'])
        ...
        RED = "\033[31m"
        RESET = "\033[0m"
        logger.info(f"Total [{RED}{cur_idx}{RESET}] samples and [{RED}{acc_tokens/1e6: .2f}{RESET} ] M tokens. ")
        return dataset.select(range(cur_idx))
    ...
    with training_args.main_process_first(desc="dataset map pre-processing"):
        ...
        if data_args.max_tokens:
            dataset = filter_by_token_limit(dataset, max_tokens=data_args.max_tokens)
