...
def get_dataset(
    ...
    for dataset_attr in data_args.dataset_list:
        ...
        if dataset_attr.load_from == "hf_hub":
            ...
            data_files: List[str] = []
            if os.path.isfile(dataset_attr.dataset_name): # if file
                data_files.append(dataset_attr.dataset_name)
                data_path = EXT2TYPE.get(dataset_attr.dataset_name.split(".")[-1], None)
            elif os.path.isdir(os.path.join(data_args.dataset_dir, dataset_attr.dataset_name)): # is directory
            ...
            if not dataset_attr.external_data:
