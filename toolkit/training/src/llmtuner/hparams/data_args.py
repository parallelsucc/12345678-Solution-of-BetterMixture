...
class DatasetAttr:
    ...
    external_data: Optional[bool] = False
...
class DataArguments:
    ...
    max_tokens: Optional[float] = field(
        default=None,
        metadata={"help": "For debugging purposes, truncate the number of tokens for each dataset."}
    )
    ...
    def __post_init__(self):
        ...
        if self.streaming and self.max_tokens is not None:
            raise ValueError("`max_tokens` is incompatible with `streaming`.")
        ...
        if not os.path.exists(self.dataset_dir):
            ...
            self.dataset_dir = os.path.join(project_dir, self.dataset_dir)
